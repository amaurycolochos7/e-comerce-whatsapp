...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {sidebarAbierta && (
        <div className="fixed inset-0 bg-black/50 z-40 lg:hidden" onClick={() => setSidebarAbierta(false)} />
      )}

      {/* Sidebar */}
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-[#0a1628] text-white flex flex-col transition-transform duration-300 lg:translate-x-0 ${sidebarAbierta ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">El Inge</p>
            </div>
          </div>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activo ? 'bg-[#4da8da] text-white shadow-lg shadow-[#4da8da]/30' : 'text-white/60 hover:bg-white/10 hover:text-white'}`}
              >
                {item.icon}
                {item.label}
              </Link>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          <a href="/" target="_blank" className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm text-white/50 hover:bg-white/5 hover:text-white/70 transition-colors">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-4 sticky top-0 z-30">
          <button onClick={() => setSidebarAbierta(true)} className="lg:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors">
